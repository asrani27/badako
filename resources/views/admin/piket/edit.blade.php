...
<div class="row">
  <div class="col-xs-12">
    ...
    <div class="box">
      ...
      <form class="form-horizontal" action="/admin/piket/edit/{{$data->id}}" method="post">
        @csrf
        <div class="box-body">
          <div class="form-group">
            ...
            <div class="col-sm-10">
              ...
              @foreach ($pegawai as $item)
                  <option value="{{$item->nip}}" {{$data->nip == $item->nip ? 'selected':''}}>{{$item->nip}} - {{$item->nama}}</option>
              @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" class="form-control" name="lama" value="{{$data->lama}}" required onkeypress="return hanyaAngka(event)">
            </div>
          </div>
        </div>
        <!-- /.box-body -->
